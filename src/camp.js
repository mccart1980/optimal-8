...
/* the shot-put ball by block: 5–6 kg through the build, 3–5 kg from the peak */
const shotBall = (rx) => (rx.w <= 6 ? "5–6 kg" : "3–5 kg");
...
export const CS = {
  ...
  tue: { n: "TUESDAY", t: "POWER + ENGINE 1 — crawls, jumps, pistols, the interval session, the settle, trunk, the Achilles hold", m: (rx) => (isTest(rx, "tue") ? 70 : 60), ac: C.cobalt,
    ...
    { L: "B", n: "Power dose — jumps", m: 8, star: 1, hard: 1, rest: "Rest 90 s", rt: 90, hide: (rx) => !!rx.tp,
      rxLine: (rx) => "broad jumps 3 × 2 · box jumps 3 × 3" + (rx.w >= 3 ? " · shuttle bursts × 6" : ""),
      items: (rx) => [{ n: "Broad jump", s: "3 × 2", cue: "Two-foot jump forward for distance, stick the landing dead still.", id: "c_broad", k: "chk" },
        ...
        .concat(rx.w >= 3 ? [{ n: "Shuttle bursts", s: "6 × 5 m out and back · 20 s between", cue: "Six times five metres out and back, a hard push-off at each turn, twenty seconds between. Three minutes.", id: "c_shuttle", k: "chk" }] : []),
      w: OUTPUT_RULE,
  ...
  wed: { n: "WEDNESDAY", t: "STRENGTH — sled, the trap bar in its phase, bench throw, ring dips, chins and the muscle-up line, Nordics, neck holds", m: 62, ac: C.oxide,
    ...
    { L: "B", n: (rx) => (campRepeatSled(rx) ? "Repeat Sled Starts" : "Heavy Sled"), m: 12, star: 1, hard: 1, hide: (rx) => !rx.sled,
      ...
      w: (rx) => (campRepeatSled(rx) ? "Forward pressure repeated on a clock." : "Get the load right by the clock: 5–7 seconds a run. Faster, add weight; slower, take some off."),
    ...
    { L: "K", n: "Drive Holds", m: 3, hard: 1, rest: "Rest 60 s", rt: 60, hide: (rx) => !rx.sled, rxLine: () => "3 × 10 seconds",
      ...
      why: "The clinch against a heavier man: the strength to hold your ground is an isometric, and this is the only place it's trained.", tr: 2 },
  ...
  thu: { n: "THURSDAY", t: "THROWS + ENGINE 2 — crawls, throws and landmine, split squat, Spanish squat hold, the second conditioning session, the settle, neck, hands", m: (rx) => (rx.nasal ? 74 : 66), ac: C.cobalt,
    ...
    { L: "B", n: "Power dose — throws + landmine", m: 8, star: 1, hard: 1, rest: "45 s between sets", rt: 45, hide: (rx) => !!rx.tp,
      ...
      items: (rx) => [{ n: "Rotational shot-put", s: "2 × 3 per side @ " + shotBall(rx), cue: "Medicine ball, " + shotBall(rx) + ", at the shoulder, side-on to the wall, drive off the back hip; flat and hard, like the punch.", id: "c_shot", k: "chk" },
  ...
  sat: { n: "SATURDAY", t: "★ THE LONG SESSION · 8:30 — get-ups, build-ups, sprints, reactive jumps, side bounds, the squat in its phase, push press, the four throws", m: (rx) => (rx.spr && rx.spr.micro ? 25 : 85), ac: C.oxide, free: 1,
    ...
    { L: "G", n: "The Four Punch Throws", m: 12, star: 1, p: "C_VEC", hide: (rx) => !!rx.spr && !!rx.spr.micro,
      ...
      w: OUTPUT_RULE,
      why: "Straights are forward drive, hooks are rotation; all four get trained. Medicine ball — 5–6 kg for the shot-put in the build and heavy blocks, 3–5 kg for the rest; if it isn't flying, it's too heavy." },
