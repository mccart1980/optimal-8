      ...
      {rows.map((r, i) => {
        ...
        return (
          <div key={r.id || i} style={{ display: "flex", gap: 10, alignItems: "baseline", padding: "7px 0", borderBottom: "1px solid " + C.line, opacity: past && !live ? .55 : 1 }}>
            <span style={Object.assign({}, mno, { fontSize: 9.5, color: live ? col : C.ash, width: 14, flexShrink: 0 })}>{live ? "▸" : i + 1}</span>
            <span style={{ flex: 1, minWidth: 0 }}>
              <div style={Object.assign({}, bdy, { fontSize: 13, fontWeight: 600, color: live ? C.chalk : C.ash, lineHeight: 1.35 })}>{r.n}</div>
              {r.s ? <div style={Object.assign({}, mno, { fontSize: 9, color: C.brass, marginTop: 2, letterSpacing: .6 })}>{r.s}</div> : null}
            </span>
          </div>); })}

      <div style={{ background: C.ink, border: "1px solid " + C.line, borderRadius: 6, padding: "12px 12px 10px", marginTop: 12 }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8 }}>
          <span style={Object.assign({}, mno, { fontSize: 32, fontWeight: 700, color: s.done ? C.moss : C.chalk, lineHeight: 1 })}>{s.done ? "✓" : mmss(s.left)}</span>
          <span style={Object.assign({}, mno, { fontSize: 9, color: C.ash, letterSpacing: 1 })}>{s.done ? mmss(R.total) + " TOTAL" : mmss(R.total - R.elapsed) + " LEFT"}</span>
        </div>
        <div style={Object.assign({}, bdy, { fontSize: 12, color: s.done ? C.moss : col, marginTop: 7, lineHeight: 1.4, fontWeight: 600 })}>{s.done ? "COMPLETE" : cur ? cur.l : ""}</div>
    ...
    <div>
      {rows.map((r, i) => (
        <div key={i} style={{ display: "flex", gap: 10, alignItems: "baseline", padding: "7px 0", borderBottom: i < rows.length - 1 ? "1px solid " + C.line : "none" }}>
          <span style={Object.assign({}, mno, { fontSize: 9.5, color: colour || C.brass, width: 14, flexShrink: 0 })}>{i + 1}</span>
          <span style={{ flex: 1, minWidth: 0 }}>
            <div style={Object.assign({}, bdy, { fontSize: 13.5, fontWeight: 600, color: C.chalk, lineHeight: 1.4 })}>{typeof r === "string" ? r : r.n}</div>
            {typeof r === "string" || !r.s ? null : <div style={Object.assign({}, mno, { fontSize: 9.5, color: C.brass, marginTop: 3, letterSpacing: .6 })}>{r.s}</div>}
            {typeof r === "string" || !r.how ? null : <div style={Object.assign({}, bdy, { fontSize: 12, color: C.ash, marginTop: 3, lineHeight: 1.45 })}>{r.how}</div>}
          </span>
        </div>))}
    </div>);
}
...
export function CheckTaps({ qs, answers, setAnswers, result }) {
  const a = answers || [];
  ...
  const line = result === "R" ? "RED — the hard steps come off the order"
    ...
    <div>
      ...
      {result ? (
        <div style={{ background: C.ink, border: "1px solid " + ac, borderRadius: 5, padding: "10px 12px", marginTop: 12 }}>
          <div style={Object.assign({}, mno, { fontSize: 11, letterSpacing: 1.4, color: ac })}>{line}</div>
        </div>) : null}
    </div>);
}
...
/* ================================================================
   ...
   ================================================================ */
const secLine = (n, c) => (
  <div style={Object.assign({}, mno, { fontSize: 9.5, letterSpacing: 2, color: c || C.brass, padding: "14px 2px 8px" })}>{n}</div>);
    ...
    <Card ac={edge} s={{ padding: 0, opacity: item.done && !open ? .62 : 1 }}>
      <div data-flow-id={item.id} data-flow-name={item.n} data-flow-open={open ? "1" : "0"} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px" }}>
        <button onClick={onTick} aria-label={(item.done ? "Untick " : "Tick ") + item.n}
          style={Object.assign({}, mno, { width: 40, height: 40, flexShrink: 0, borderRadius: 5, cursor: "pointer", fontSize: 13, fontWeight: 700, background: item.done ? C.moss : C.ink, color: item.done ? C.ink : col, border: "1px solid " + (item.done ? C.moss : C.line) })}>{item.done ? "✓" : no}</button>
        <span onClick={onOpen} style={{ flex: 1, minWidth: 0, cursor: "pointer" }}>
          {item.clock ? <div style={Object.assign({}, mno, { fontSize: 9, color: C.ash, letterSpacing: 1 })}>{item.clock}</div> : null}
          <div style={Object.assign({}, bdy, { fontSize: 14.5, fontWeight: 600, color: C.chalk, lineHeight: 1.25 })}>{item.n}</div>
          {item.s ? <div style={Object.assign({}, mno, { fontSize: 9.5, color: C.brass, marginTop: 3, lineHeight: 1.4 })}>{item.s}</div> : null}
        </span>
        <span onClick={onOpen} style={Object.assign({}, mno, { fontSize: 13, color: col, flexShrink: 0, cursor: "pointer", width: 18, textAlign: "right" })}>{open ? "▾" : "▸"}</span>
      </div>
      {open ? (
...
export function TodayFlow({ items, curId, go, mark, streak, header }) {
  ...
  return (
    <div>
      ...
      {allDone ? (
        <Card ac={C.moss}>
          <div style={Object.assign({}, dsp, { fontSize: 30, fontWeight: 800, letterSpacing: 2, color: C.moss, lineHeight: 1 })}>DONE</div>
          <div style={Object.assign({}, mno, { fontSize: 10, color: C.ash, letterSpacing: 1.4, marginTop: 8 })}>STREAK · {streak == null ? "—" : streak}</div>
        </Card>) : null}
      {live.map((it, i) => {
        no += 1;
